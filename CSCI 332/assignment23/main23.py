...
def FindConvexHull(points):
    ...
    for p in points:
        ...
        if orientation(leftmost, rightmost, p) > 0:     #finding the leftmost point and rightmost and putting that into groups
            leftPoints.append(p)
        else:
            rightPoints.append(p)

    leftHull = FindConvexHull(leftPoints)
    rightHull = FindConvexHull(rightPoints)

    combined = leftHull + rightHull + [leftmost, rightmost]
